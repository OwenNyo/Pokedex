...
// Extract ability ID from API URL
...
export default function AbilitiesTab() {
  ...
  // Fetch paginated ability list when page changes
  useEffect(() => {
    let alive = true;
  ...
  // Fetch full ability list for global search
  useEffect(() => {
    let alive = true;
    ...
    async function loadAllAbilities() {
      ...
      try {
        const res = await P.getAbilitiesList({ offset: 0, limit: 100000 });

        if (!alive || reqId !== searchRequestIdRef.current) return;
        setAllAbilities(res?.results ?? []);
      } catch (err) {
        if (!alive || reqId !== searchRequestIdRef.current) return;
        setErrorMsg(err?.message || "Failed to load searchable ability list.");
      } finally {
        if (!alive || reqId !== searchRequestIdRef.current) return;
  ...
  }, [isSearching, allAbilities.length]);
  ...
  const filtered = useMemo(() => {
    ...
    return allAbilities.filter((ability) => ability.name.includes(q));
  }, [data, allAbilities, query]);
  ...
  return (
    <section className="p-6 text-left">
      ...
      <p className="text-sm text-slate-300 mt-1">
        Browse abilities. Search looks across all pages.
      </p>
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="w-full sm:max-w-md">
          ...
          <input
            ...
            placeholder="Search all pages (e.g. blaze)"
